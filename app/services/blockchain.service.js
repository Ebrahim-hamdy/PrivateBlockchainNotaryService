...
class Blockchain {
    constructor() {
        leveldb.getLevelDBHeight().then(blockHeight => {
            if(blockHeight === 0) {
                let blockBody = {
                    address: 'genisis-block-address',
                    star: {dec: '', ra: '', story: 'Star Registry story'}
                };
    ...
    async getBlockByHash(hash) {
        return await leveldb.getBlockByHashFromDB(hash);
    }

    // get block by address
    async getBlockByAddress(address) {
        return await leveldb.getBlocksByAddressFromDB(address);
    }
}
